...
public class GlitchPlayerController : MonoBehaviour
{
    ...
    [Header("Animator")]
    [Tooltip("True while moving upward in the air → PlayerJump.")]
    public string isJumpingParameter = "isJumping";
    [Tooltip("True while falling (or apex) in the air → PlayerFall.")]
    public string isFallingParameter = "isFalling";
    [Tooltip("Vertical speed above this counts as jump; at or below counts as fall (apex uses fall).")]
    public float verticalAnimThreshold = 0.05f;
    ...
    void Start()
    {
        ...
        if (animator != null)
        {
            if (!string.IsNullOrEmpty(isJumpingParameter))
                isJumpingHash = Animator.StringToHash(isJumpingParameter);
            if (!string.IsNullOrEmpty(isFallingParameter))
    ...
    void Update()
    {
        ...
        if (animator != null)
        {
            ...
            {
                if (isJumpingHash != 0)
                    animator.SetBool(isJumpingHash, vy > verticalAnimThreshold);
                if (isFallingHash != 0)
                    animator.SetBool(isFallingHash, vy <= verticalAnimThreshold);
            }
            else
            {
                if (isJumpingHash != 0)
